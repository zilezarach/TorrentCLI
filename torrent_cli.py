...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "qbittorrent": {"host": "http://127.0.0.1:8080", "username": "zile", "password": "zile123"},
...
def ensure_files() -> None:
    ...
    Path(cfg.get("direct_download_path", DEFAULT_CONFIG["direct_download_path"])).mkdir(parents=True, exist_ok=True)
...
def qb_client() -> QBClient:
    ...
    qb.auth_log_in()
    return qb
def fetch(query: str, limit: int = 10, category: Optional[str] = None) -> List[SearchResult]:
...
def schedule_runner() -> None:
    ...
    while True:
        ...
        updated = False
        for task in sched:
            if not task.get("done") and datetime.fromisoformat(task["time"]) <= datetime.now():
...
def find_torrent_by_name(
    ...
    for attempt in range(max_wait):
        ...
        for t in all_torrents:
            ...
            if (search_name_lower in torrent_name or
                torrent_name in search_name_lower or
                any(word in torrent_name for word in search_name_lower.split() if len(word) > 3)):
                return t.hash
        time.sleep(1)
    return None
def download_direct_file(result: Dict[str, Any]) -> bool:
    ...
    """
    cfg = load_json(CONFIG_PATH)
    download_path = Path(cfg.get("direct_download_path", DEFAULT_CONFIG["direct_download_path"]))
    download_path.mkdir(parents=True, exist_ok=True)
   
    # Create filename from title and extension
    extra = result.get('extra', {})
    extension = extra.get('extension', 'pdf')
    safe_title = "".join(c for c in result.get('title', '') if c.isalnum() or c in (' ', '-', '_')).strip()
    ...
    try:
        ...
        console.print(f"[dim]→ Getting download URL (source=auto, hint={source_hint or 'none'})...[/dim]")
        download_url_info = api.get_download_url(md5, source="auto", source_hint=source_hint)
        download_url = download_url_info.get('download_url') or download_url_info.get('mirror')
        if not download_url:
            console.print(Panel(
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total_size > 0:
                            ...
                            progress.update(task, completed=(downloaded / (1024*1024)) % 100)
...
def download_with_progress(item: Dict[str, Any]) -> None:
    ...
    with Progress(
        ...
        while not progress.finished:
            ...
            progress.update(
                ...
                download_speed=format_speed(info.dlspeed) if info.dlspeed is not None else "0 B/s",
                upload_speed=format_speed(info.upspeed) if info.upspeed is not None else "0 B/s",
                state=current_state
            )
            ...
            elif info.progress >= 1.0 and info.state in ["uploading", "stalledUP", "queuedUP"]:
                break
    ...
    if cfg.get("auto_remove_completed") and task_hash:
        try:
            ...
            console.print("[dim]→ Automatically removed from qBittorrent (files kept)[/dim]")
        except Exception as e:
...
def browse_interactive() -> None:
    ...
    if not results:
        console.print(Panel("No results found", title="[bold yellow]No Results[/bold yellow]", border_style="yellow"))
        return
    ...
    while True:
        ...
        for i, r in enumerate(page_items, start=start + 1):
            download_type_icon = "📦" if r.download_type == DownloadType.DIRECT else "🧲"
        ...
        action = Prompt.ask(
            ...
            choices=["n", "p", "q"] + [str(i) for i in range(start + 1, start + 1 + len(page_items))],
        )
            ...
            try:
                ...
                if Confirm.ask("[bold cyan]Download this?[/bold cyan]"):
                    ...
                    item_dict = {
                        ...
                        'MagnetUri': item.download_url if item.download_type != DownloadType.DIRECT else None,
                    ...
                    for r in results:
                        ...
                        if 'download_type' in r_dict and isinstance(r_dict['download_type'], DownloadType):
...
def list_downloads(
    ...
    with console.status("[bold green]Fetching torrents...", spinner="dots"):
        qb = qb_client()
        torrents = qb.torrents_info() if all_torrents else qb.torrents_info(filter="downloading")
...
@app.command()
def browse_category(
    category: str = typer.Argument(..., help="Category to browse (movies, books, games)"),
    limit: int = 20
) -> None:
...
def dashboard() -> None:
    ...
    try:
        ...
        with console.status("[bold green]Loading dashboard...", spinner="dots"):
            ...
            active_torrents = [t for t in torrents if t.state in ["downloading", "uploading"]]
        ...
        header_text.append(f" | 📡 Indexers: {health.get('healthy_count')}/{health.get('total_indexers')}", style="cyan")
        ...
        if recent_hist:
            for entry in reversed(recent_hist):
                time_str = datetime.fromtimestamp(entry.get("time", 0)).strftime("%m/%d %H:%M")
...
@app.command()
def remove(
    hash_or_name: str = typer.Argument(..., help="Torrent hash or part of the name to remove"),
    delete_files: bool = typer.Option(False, "--files", "-f", help="Also delete associated files"),
) -> None:
    """🗑️ Remove a torrent by its hash or part of its name."""
    qb = qb_client()
    torrents_to_remove = []
   
    if len(hash_or_name) == 40 and all(c in "0123456789abcdefABCDEF" for c in hash_or_name):
    ...
    if Confirm.ask(Panel(warning_text, title="[bold red]Confirm Removal[/bold red]", border_style="red")):
        qb.torrents_delete(torrent_hashes=selected_torrent.hash, delete_files=delete_files)
        console.print(Panel(
        ...
        console.print("[yellow]Removal cancelled[/yellow]")
@app.command()
def pause(hash_or_name: str = typer.Argument(..., help="Torrent hash or part of the name to pause")) -> None:
...
@app.command()
def resume(hash_or_name: str = typer.Argument(..., help="Torrent hash or part of the name to resume")) -> None:
...
def set_priority(
    ...
    priority: int = typer.Argument(0, help="Priority (0=Do not download, 1=Normal, 2=High, etc.)"),
) -> None:
    ...
    if len(torrents_to_prioritize) > 1:
        ...
        while True:
            choice = Prompt.ask(
                ...
                choices=[str(i + 1) for i in range(len(torrents_to_prioritize))] + ["q"],
            )
            if choice == "q":
...
def api_status() -> None:
    ...
    try:
        ...
        console.print(Panel(f"Failed to connect to API: {e}", title="[bold red]✗ Error[/bold red]", border_style="red"))
@app.command()
def history() -> None:
    ...
    for e in reversed(hist[-50:]): # Last 50 entries
        time_str = datetime.fromtimestamp(e.get("time", 0)).strftime("%Y-%m-%d %H:%M") \
            if e.get("time") is not None else "Unknown"
        tbl.add_row(
            time_str,
...
@app.command()
def set_config(
    key: str = typer.Argument(..., help="Configuration key (e.g., download_path or qbittorrent.host)"),
        ...
        if i == len(keys) - 1:
            try:
                if isinstance(temp_cfg, dict):
                    ...
                    if k in ["health_check_interval", "schedule_check_interval", "max_active_downloads"]:
...
def schedule_add(
    ...
    limit: int = typer.Option(5, "--limit", "-l", help="Limit results for the scheduled search"),
) -> None:
...
def schedule_list() -> None:
    ...
    for i, task in enumerate(sched, 1):
        ...
        tbl.add_row(
            str(i),
            safe_str(task.get("query")),
            safe_str(task.get("time")),
            status
        )
   
    console.print(tbl)
@app.command()
def schedule_remove(index: int = typer.Argument(..., help="Index of the scheduled task to remove")) -> None:
...
if __name__ == "__main__":
    # Start background threads for health monitoring and schedule checking
