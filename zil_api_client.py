...
class SearchResult:
    ...
    def from_api_response(cls, data: Dict[str, Any]) -> 'SearchResult':
        ...
        download_type_str = extra.get('download_type') or data.get('download_type', 'torrent')
        ...
        if download_type == DownloadType.DIRECT:
            download_url = extra.get('mirror', '') or data.get('magnet_uri', '') or data.get('link', '')
        else:
...
class GoTorrentAPI:
    ...
    def _make_request(self, method: str, path: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise APIError(f"Could not connect to API at {self.base_url}. Make sure the server is running.")
    ...
    def search_movies(self, query: str, limit: int = 25) -> List[SearchResult]:
        ...
        Returns:
            List of SearchResult objects
        """
        return self.search(query, limit, category=Category.MOVIES.value)
    
    def search_books(self, query: str, limit: int = 25, source: str = "both") -> List[SearchResult]:
        """
    ...
    def search_games(self, query: str, limit: int = 20) -> List[SearchResult]:
        ...
        """
        params = {'query': query, 'limit': limit}
        response = self._make_request('GET', '/api/v1/games/repacks/search', params=params)
        data = response.json()
    ...
    def get_latest_games(self, limit: int = 30) -> List[SearchResult]:
        ...
        """
        params = {'limit': limit}
        response = self._make_request('GET', '/api/v1/games/repacks/latest', params=params)
        data = response.json()
        ...
        for item in data.get('results', []):
            try:
                ...
                continue
        return results
    def get_download_url(self, md5: str, source: str = "auto", source_hint: str = "") -> Dict[str, Any]:
        params = {'md5': md5}
    ...
    def fetch(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for result in results:
            item = {
                ...
                'MagnetUri': result.download_url if not result.is_direct_download() else None,
                'Link': result.download_url if result.is_direct_download() else result.download_url,
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Healthy indexers: {health.get('healthy_count')}/{health.get('total_indexers')}")
        print()
        ...
        for i, result in enumerate(movie_results, 1):
            print(f"{i}. 🎥 {result.title[:70]}")
            print(f"   Size: {result.size} | Seeders: {result.seeders} | Source: {result.source}")
            print()
        ...
        if book_results and book_results[0].info_hash:
            ...
            try:
                ...
                print(f"Download URL: {download_info.get('download_url', 'N/A')[:80]}...")
